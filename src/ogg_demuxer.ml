    ...
    Hashtbl.iter
      (fun format (check,decode) ->
           ...
           if check packet then
             (log dec "ogg/%s format detected for stream %nx" 
                format serial ;
              raise (Exit (serial,os,decode os)))
           else ())
      ogg_decoders;
    log dec "Couldn't find a decoder for ogg logical \
  ...
  try
    ...
    if stream.dec <> Unknown then
     begin
      ...
      in
      if total_samples > stream.read_samples then
       begin
        begin
         match position with
           | Some p ->
...
let init ?(log=(fun _ -> ())) c = 
  ...
  let pos =
    match c.tell with
      ...
      | Some f -> Some (f ())
  in
  parse { sync = sync; started = false ; log = log;
          streams = streams; callbacks = c; 
          last_p = pos;
...
let init_from_fd ?log fd =
  init ?log (unix_callbacks fd)
...
let reset dec = 
  ...
  log dec "Reseting a stream that has not ended!" ;
...
let get_standard_tracks ?tracks dec = 
  let f id stream (a_t,v_t,l) = 
    match stream.dec with
      | Audio d when a_t = None ->
          Some (Audio_track (d.name,id)),v_t,l
      | Audio d -> 
          a_t,v_t,(Audio_track (d.name,id)) :: l
      | Video d when v_t = None ->
          a_t,Some (Video_track (d.name,id)),l
      | Video d -> 
          a_t,v_t,(Video_track (d.name,id)) :: l
      | _ -> a_t,v_t,l
  in
...
let update_standard_tracks dec tracks = 
  ignore(get_standard_tracks ~tracks dec)

let get_standard_tracks dec = 
  get_standard_tracks dec

let rec sample_rate_priv d dec =
  try
    match d with
      | Audio d ->
          ((fst(d.info ())).sample_rate,1)
      | Video d ->
          ((fst(d.info ())).fps_numerator,
           (fst(d.info ())).fps_denominator)
      | _ -> assert false
  with
    | Ogg.Not_enough_data ->
        feed dec; sample_rate_priv d dec
...
let get_position dec = 
  ...
  let f _ stream pos =
    match stream.dec with
      | Audio _ 
      | Video _ -> 
          min stream.position pos
      | _ -> pos
  in
  fold_tracks dec f max_float

let can_seek dec = 
  dec.callbacks.seek <> None &&
  dec.callbacks.tell <> None
...
let find_seek_pos dec time sync_point =
  let samples = 
    Int64.of_float (time *. sync_point.sync_rate)
  in
  while sync_point.sync_stream.read_samples <= samples do
    feed dec
  done;
  let f granulepos index_element = 
    if index_element.total_samples <= samples &&
       Int64.add 
         index_element.total_samples 
         index_element.samples >= samples then 
      raise (Position (granulepos,index_element))
  in
  ...
  in
  let skip_samples = 
     Int64.sub samples index_element.total_samples
  in
...
let sync_forward dec sync_points sync_point =
  let rec skip (cur,skipped) =
    try
      let pos = 
          Ogg.Stream.peek_granulepos sync_point.sync_stream.os
      in
      let total_samples = 
        granuleconv sync_point.sync_stream.dec pos cur 
      in
      let diff =
        Int64.to_int (Int64.sub total_samples cur)
      in
      if skipped+diff < sync_point.sync_skip_samples then
       begin
        ...
        sync_point.sync_stream.position <- 
          (Int64.to_float sync_point.sync_stream.read_samples +. 
           float skipped) /. sync_point.sync_rate
    with
      | Ogg.Out_of_sync -> skip (cur,skipped)
      | Ogg.Not_enough_data ->
                  feed_sync dec sync_points; skip (cur,skipped)
  in
...
let seek ?(relative=false) dec time = 
  ...
  let time =
    if time < 0. then
      ...
      time
  in
  log dec "Seeking to absolute position at \
           %.2f sec" time;
  let f id stream l =
    let sample_rate () = 
  ...
  let sync_bytes = 
    let f cur sync_point = 
      if sync_point.sync_bytes < cur then
        sync_point.sync_bytes
      else
        cur
    in
  ...
  in
  let page = 
    sync_seek dec sync_bytes 
  in
  (* First, reinitiate all ogg streams. *)
  let reiniate x =
    x.sync_stream.os <- 
        Ogg.Stream.create ~serial:x.sync_id ();
    if Ogg.Page.serialno page = x.sync_id then
      feed_sync_page x page
  in
